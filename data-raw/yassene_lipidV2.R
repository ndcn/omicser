...
prep_lipidizer_files <- function(data_file,path_root){
    ...
    raw_table <- fread(file=file.path(path_root,data_file), header=TRUE)
    ...
    var_annot$var <- colVars(raw,na.rm = TRUE)

    obs_meta$var <- rowVars(raw,na.rm = TRUE)
  ...
  conc <- setup_database(database_name=DB_NAME, data_in=conc_dat_list, db_meta=NULL , re_pack=TRUE)
  comp <- setup_database(database_name=DB_NAME, data_in=comp_dat_list, db_meta=NULL , re_pack=TRUE)
  ...
  ######TODO:  the composion tables are normalized already...
  ######        so we might want to do some alternative stats...

  #==== 5-b. dimension reduction - PCA / umap =========================================================================
  require(anndata)
  require(reticulate)
