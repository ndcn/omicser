...
Ingesting raw data requires the use of the `scanpy` Python package,
which we'll access using the `reticulate` R package.
...
This step involves reading the PBMC3k data files using `scanpy`'s `read_10x_mtx()` function,
then writing the data to file in `.h5ad` format.
This format is the database format we'll use for eventual visualization in the browser.
First, though, we'll need to perform some additional manipulations to add structure to the data.
...
You might also want to save some _intermediate_ files such as in the [examples](examples/browse_pbmc3k.R) which also generate: `normalized_data.h5ad`,`core_data.h5ad`,and `norm_data_plus_dr.h5ad`.
...
FIXME: excecuting code within notebooks is tricky, use scripts instead
